...
class Model(nn.Module):
  ...
  # The forward method should be defined as a method of the class
  def forward(self,x):
...
y1 = x1*final_weight + final_bias # Changed final_weigt to final_weight

plt.scatter(X.numpy(), y.numpy(),label = 'Original Data') # Changed X.numpt() to X.numpy()
plt.plot(x1.numpy(), y1.numpy(), 'r',label = 'Best Fit Line') # Changed 'r' to 'r-' and added label
plt.xlabel('X')
plt.ylabel('y')
